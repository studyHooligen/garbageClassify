    ...
    while(1)
    {
        ...
        if (rt_mb_recv(&gbgTr_mb, (rt_uint32_t *)&pos, RT_WAITING_FOREVER) == RT_EOK)
        {
            switch(pos)
            {
                ...
                case(1):
                    // 扫把扫描一圈
                    steerEngine_SetSpeed(1,SlowAhead);
                    rt_thread_mdelay(SCAN_NVIC_DELAY);
                    HAL_NVIC_EnableIRQ(SCANO_EXTI5_EXTI_IRQn);
                
                    break;
                case(2):
                    // 圆盘转至目标位置
                    steerEngine_SetSpeed(0,SlowAhead);
                    while(rt_mb_recv(pos_mb,(rt_uint32_t *)&pos,RT_WAITING_FOREVER) == RT_EOK)
                    {
                        if(pos == 2)
                            break;
                    }
                    steerEngine_SetSpeed(0,Stop);
                    ...
                    // 圆盘回位
                    steerEngine_SetSpeed(0,FullAhead);
                    while(rt_mb_recv(pos_mb,(rt_uint32_t *)&pos,RT_WAITING_FOREVER) == RT_EOK)
                    {
                        if(pos == 1)
                            break;
...
rt_mailbox_t threadInit_GarbageThrow(void)
{
    ...
    /* 建立静态线程 */
    rt_thread_init(&threadGarbageThrow_Source,
                "garbage throw",
